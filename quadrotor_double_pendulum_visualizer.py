...
class QuadrotorDoublePendulumVisualizer(PyPlotVisualizer):

    def __init__(self, quadrotor_pendulum):
        PyPlotVisualizer.__init__(self, figsize = (10,10))
...
#         self.screen_size = 25
        ...
        self.pendulum1_joint_pts = populate_circle_vertices(self.pendulum_radius*4, 10)
        self.pendulum1_joint_pts[1,:] -= 2*self.l1
        self.pendulum1_joint = self.ax.fill(self.pendulum1_joint_pts[0, :], self.pendulum1_joint_pts[1, :], zorder=3, facecolor='tab:blue', edgecolor='k', closed=True)
        
        self.l2 = quadrotor_pendulum.l2
        self.pendulum2_pts = populate_rectangle_vertices(self.pendulum_radius, self.l2)
        self.pendulum2_pts[1,:] -= self.l2
        self.pendulum2 = self.ax.fill(self.pendulum2_pts[0, :], self.pendulum2_pts[1, :], zorder=1, facecolor=(.0, .0, .0), edgecolor='k', closed=True)
        
        self.pendulum2_com_pts = populate_circle_vertices(self.pendulum_radius*4, 10)
        self.pendulum2_com_pts[1,:] -= self.l2
        self.pendulum2_com = self.ax.fill(self.pendulum2_com_pts[0, :], self.pendulum2_com_pts[1, :], zorder=3, facecolor='tab:red', edgecolor='k', closed=True)
    ...
    def draw(self, context):
        if isinstance(context, Context):
            ...
            theta2 = self.EvalVectorInput(context, 1).get_value()[4]
        ...
        path = self.pendulum1_joint[0].get_path()
        path.vertices[:,:] = np.dot(rotmat_theta1, self.pendulum1_joint_pts).T
        path.vertices[:,0] += (xb-xc)
        path.vertices[:,1] += (yb-yc)
        
        path = self.pendulum2[0].get_path()
        path.vertices[:,:] = np.dot(rotmat_theta2, self.pendulum2_pts).T
        path.vertices[:,0] += (xb-xc)
        path.vertices[:,1] += (yb-yc)
        path.vertices[:,0] += 2.*self.l1*math.sin(theta1)
        path.vertices[:,1] -= 2.*self.l1*math.cos(theta1)
        
        path = self.pendulum2_com[0].get_path()
        path.vertices[:,:] = np.dot(rotmat_theta2, self.pendulum2_com_pts).T
        path.vertices[:,0] += (xb-xc)
        path.vertices[:,1] += (yb-yc)
        path.vertices[:,0] += 2.*self.l1*math.sin(theta1)
        path.vertices[:,1] -= 2.*self.l1*math.cos(theta1)
